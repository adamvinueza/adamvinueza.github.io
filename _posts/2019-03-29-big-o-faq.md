...
- *Logarithmic*: O(_log n_) indicates that the number of operations increases as a
  ...
  operations is no larger than log (base 2) of _n_.
...
Many numbers satisfy this inequality; therefore, `LinkedList.search` is
O(_n_). QED<sup>[\*](#qedfootnote)</sup>.
...
<a name="qedfootnote">I took a mathematics class in which the professor ended
each of his proofs with the acronym _ATWMR_, which stood for "And there was much
rejoicing."
